...
## Google Tasks 연결 (OAuth)
1. Google Cloud에서 `Google Tasks API` 활성화
2. OAuth 동의 화면 설정
3. OAuth 클라이언트(`Web application`) 생성
4. Redirect URI 추가
- `https://<project>.vercel.app/api/google/tasks/callback`
5. 아래 값을 Vercel 환경변수에 입력
- `GOOGLE_CLIENT_ID`
- `GOOGLE_CLIENT_SECRET`
- `GOOGLE_REDIRECT_URI`
6. Discord에서 `/gtasks_auth` 실행 후 링크 열기
